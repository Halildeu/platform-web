...
export const MODULE_KEYS = {
   ...
   * `usePermissions().hasModule()` (backend-mapped from `/v1/authz/me`
   * modules array). Distinct from the OpenFGA tuple object id, which is
   * kebab-case `module:endpoint-admin` (see EndpointAdminAuthz.MODULE
   * on the backend). The auth-service is responsible for keeping the
   * two in sync per Faz 22.1.1b.
   */
