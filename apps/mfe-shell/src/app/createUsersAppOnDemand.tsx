...
const USERS_HOST_NAME = 'mfe_shell';
...
function getHostMfInstance(): MfHostInstance | null {
  ...
  return instances.find((i) => i.options.name === USERS_HOST_NAME) ?? null;
}
...
async function loadUsersRemote(): Promise<{ default: FC<PropsWithChildren> }> {
  ...
  if (!host) {
    throw new Error(
      `[B5b2-prep] Host MF runtime instance "${USERS_HOST_NAME}" disappeared between ensure and load.`,
    );
  }
