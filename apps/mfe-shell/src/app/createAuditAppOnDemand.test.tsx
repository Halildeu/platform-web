...
/**
 * PERF-INIT-V2 PR-B5b2-prep-2 — `mfe_audit` on-demand bootstrap canary
 ...
 *      `host.registerRemotes` + `host.loadRemote('mfe_audit/shell-services')`
 *      + `mod.configureShellServices(sharedServices)`, THEN the wrapper
 *      calls `host.loadRemote('mfe_audit/AuditApp')` to render the
 *      route component.
 ...
 *   6. Module-shape: `displayName`, `AUDIT_ON_DEMAND_BUILD_FLAG`.
 *   7. `resolveAuditRemoteEntry` env precedence.
 *
...
function installFakeHost(opts?: {
  ...
  const fake: FakeHostInstance = {
    ...
    loadRemote: vi.fn(async (key: string) => {
      ...
      // `${remoteName}/AuditApp` key
      if (opts?.appComponentOverride === null) return null;
      return {
        default:
          opts?.appComponentOverride ??
          (() => <div data-testid="audit-remote-loaded">AuditApp loaded</div>),
      };
    }),
  };
...
describe('createAuditAppOnDemand (PR-B5b2-prep-2)', () => {
  beforeEach(async () => {
  ...
  it('first mount: ensure helper registers + loads shell-services + configures, then wrapper loads AuditApp', async () => {
    const host = installFakeHost();
    const mod = await import('./createAuditAppOnDemand');

    render(
      <Suspense fallback={<div>Loading</div>}>
        <mod.AuditAppOnDemand />
      </Suspense>,
    );

    expect(await screen.findByTestId('audit-remote-loaded')).toBeInTheDocument();

    // Helper calls registerRemotes ONCE with the audit entry.
    ...
    // loadRemote called twice — shell-services (via helper), then AuditApp (via wrapper).
    expect(host.loadRemote).toHaveBeenCalledWith('mfe_audit/shell-services');
    expect(host.loadRemote).toHaveBeenCalledWith('mfe_audit/AuditApp');
    expect(host.loadRemote).toHaveBeenCalledTimes(2);

    // Helper's configured-remotes Set marks mfe_audit as configured.
    expect(await isConfigured('mfe_audit')).toBe(true);
  });
  ...
  it('idempotent: re-mount does NOT re-trigger ensure helper register', async () => {
    ...
    await screen.findByTestId('audit-remote-loaded');
    unmount();
  ...
  it('missing host instance surfaces classified remote-unavailable fallback', async () => {
    ...
    expect(await screen.findByTestId('remote-module-fallback-audit')).toBeInTheDocument();
    expect(warnSpy).toHaveBeenCalled();
    // Failure does NOT mark the remote as configured — retry can succeed.
    expect(await isConfigured('mfe_audit')).toBe(false);
  });
  ...
  it('AuditAppOnDemand has correct displayName', async () => {
    const mod = await import('./createAuditAppOnDemand');
    expect(mod.AuditAppOnDemand.displayName).toBe('AuditAppOnDemand');
  });

  it('exports AUDIT_ON_DEMAND_BUILD_FLAG', async () => {
    const mod = await import('./createAuditAppOnDemand');
    expect('AUDIT_ON_DEMAND_BUILD_FLAG' in mod).toBe(true);
  });

  it('uses VITE_MFE_AUDIT_URL when set', async () => {
    process.env.VITE_MFE_AUDIT_URL = 'http://example.test/custom-remote-entry.js';
    const host = installFakeHost();
    const mod = await import('./createAuditAppOnDemand');
  ...
  it('runtime MFE_AUDIT_URL takes precedence over VITE_MFE_AUDIT_URL', async () => {
    process.env.MFE_AUDIT_URL = 'http://runtime.example.test/remoteEntry.js';
    process.env.VITE_MFE_AUDIT_URL = 'http://build.example.test/remoteEntry.js';
    const host = installFakeHost();
    const mod = await import('./createAuditAppOnDemand');
  ...
  it('falls back to localhost:3006 when no env URL is set', async () => {
    delete process.env.MFE_AUDIT_URL;
    delete process.env.VITE_MFE_AUDIT_URL;
    const host = installFakeHost();
    const mod = await import('./createAuditAppOnDemand');
    ...
    const [remotes] = host.registerRemotes.mock.calls[0];
    expect(remotes[0].entry).toBe('http://localhost:3006/remoteEntry.js');
  });
});
