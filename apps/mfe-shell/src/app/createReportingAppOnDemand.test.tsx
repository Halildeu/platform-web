 ...
 *      `host.registerRemotes` + `host.loadRemote('mfe_reporting/shell-services')`
 *      + `mod.configureShellServices(sharedServices)`, THEN the wrapper
 *      calls `host.loadRemote('mfe_reporting/ReportingApp')` to render the
 *      route component.
 ...
 *   6. Module-shape: `displayName`, `REPORTING_ON_DEMAND_BUILD_FLAG`.
 *   7. `resolveReportingRemoteEntry` env precedence.
 *
...
function installFakeHost(opts?: {
  ...
  const fake: FakeHostInstance = {
    ...
    loadRemote: vi.fn(async (key: string) => {
      ...
      // `${remoteName}/ReportingApp` key
      if (opts?.appComponentOverride === null) return null;
      return {
        default:
          opts?.appComponentOverride ??
          (() => <div data-testid="reporting-remote-loaded">ReportingApp loaded</div>),
      };
    }),
  };
...
describe('createReportingAppOnDemand (PR-B5b2-prep-2)', () => {
  beforeEach(async () => {
  ...
  it('first mount: ensure helper registers + loads shell-services + configures, then wrapper loads ReportingApp', async () => {
    const host = installFakeHost();
    const mod = await import('./createReportingAppOnDemand');

    render(
      <Suspense fallback={<div>Loading</div>}>
        <mod.ReportingAppOnDemand />
      </Suspense>,
    );

    expect(await screen.findByTestId('reporting-remote-loaded')).toBeInTheDocument();

    // Helper calls registerRemotes ONCE with the reporting entry.
    ...
    // loadRemote called twice — shell-services (via helper), then ReportingApp (via wrapper).
    expect(host.loadRemote).toHaveBeenCalledWith('mfe_reporting/shell-services');
    expect(host.loadRemote).toHaveBeenCalledWith('mfe_reporting/ReportingApp');
    expect(host.loadRemote).toHaveBeenCalledTimes(2);

    // Helper's configured-remotes Set marks mfe_reporting as configured.
    expect(await isConfigured('mfe_reporting')).toBe(true);
  });
  ...
  it('idempotent: re-mount does NOT re-trigger ensure helper register', async () => {
    ...
    await screen.findByTestId('reporting-remote-loaded');
    unmount();
  ...
  it('missing host instance surfaces classified remote-unavailable fallback', async () => {
    ...
    expect(await screen.findByTestId('remote-module-fallback-reporting')).toBeInTheDocument();
    expect(warnSpy).toHaveBeenCalled();
    // Failure does NOT mark the remote as configured — retry can succeed.
    expect(await isConfigured('mfe_reporting')).toBe(false);
  });
  ...
  it('ReportingAppOnDemand has correct displayName', async () => {
    const mod = await import('./createReportingAppOnDemand');
    expect(mod.ReportingAppOnDemand.displayName).toBe('ReportingAppOnDemand');
  });

  it('exports REPORTING_ON_DEMAND_BUILD_FLAG', async () => {
    const mod = await import('./createReportingAppOnDemand');
    expect('REPORTING_ON_DEMAND_BUILD_FLAG' in mod).toBe(true);
  });

  it('uses VITE_MFE_REPORTING_URL when set', async () => {
    process.env.VITE_MFE_REPORTING_URL = 'http://example.test/custom-remote-entry.js';
    const host = installFakeHost();
    const mod = await import('./createReportingAppOnDemand');
  ...
  it('runtime MFE_REPORTING_URL takes precedence over VITE_MFE_REPORTING_URL', async () => {
    process.env.MFE_REPORTING_URL = 'http://runtime.example.test/remoteEntry.js';
    process.env.VITE_MFE_REPORTING_URL = 'http://build.example.test/remoteEntry.js';
    const host = installFakeHost();
    const mod = await import('./createReportingAppOnDemand');
  ...
  it('falls back to localhost:3007 when no env URL is set', async () => {
    delete process.env.MFE_REPORTING_URL;
    delete process.env.VITE_MFE_REPORTING_URL;
    const host = installFakeHost();
    const mod = await import('./createReportingAppOnDemand');
    ...
    const [remotes] = host.registerRemotes.mock.calls[0];
    expect(remotes[0].entry).toBe('http://localhost:3007/remoteEntry.js');
  });
});
