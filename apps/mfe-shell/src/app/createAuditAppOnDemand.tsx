 ...
 *
 *   1. `apps/mfe-shell/vite.config.ts` omits the `mfe_audit` entry from
 ...
 *      `import('mfe_audit/AuditApp')` specifier for the
 *      `AuditAppOnDemand` component declared below.
 *   3. `shell-services-wiring.ts` swaps the static
 *      `import('mfe_audit/shell-services')` line for an idle-deferred
 *      `ensureRemoteShellServicesConfigured('mfe_audit', ...)` call so
 *      the shell-services contract (notify / SSE / impersonation / auth
 *      ready) is still wired post-auth without the eager fetch.
 *   4. On first navigation to `/audit`, the lazy factory FIRST awaits
 ...
 *      `mfe_audit/AuditApp` from the (now registered) remote.
 *
 ...
 * `import('mfe_audit/AuditApp')` specifier no longer exists in the
 ...
 */
function resolveAuditRemoteEntry(): string {
  if (typeof window !== 'undefined') {
    const w = window as Window & { __env__?: Record<string, string> };
    const url = w.__env__?.MFE_AUDIT_URL ?? w.__env__?.VITE_MFE_AUDIT_URL ?? null;
    if (url) return url;
  }
  if (typeof process !== 'undefined' && process.env) {
    const url = process.env.MFE_AUDIT_URL ?? process.env.VITE_MFE_AUDIT_URL ?? null;
    if (url) return url;
  }
  // Dev fallback — matches default in apps/mfe-shell/vite.config.ts.
  return 'http://localhost:3006/remoteEntry.js';
}
 ...
 *      — registers the remote + loads `mfe_audit/shell-services` +
 ...
 *   3. `host.loadRemote('mfe_audit/AuditApp')` — remote is now
 ...
 * pattern).
 */
async function loadAuditRemote(): Promise<{ default: FC<PropsWithChildren> }> {
  const sharedServices = getSharedShellServices();
  await ensureRemoteShellServicesConfigured(
    AUDIT_REMOTE_NAME,
    resolveAuditRemoteEntry(),
    sharedServices,
  );
  ...
  if (!host) {
    throw new Error(
      `[B5b2-prep] Host MF runtime instance "${AUDIT_HOST_NAME}" disappeared between ensure and load.`,
    );
  }
  const mod = await host.loadRemote<{ default: FC<PropsWithChildren> }>(AUDIT_REMOTE_KEY);
  if (!mod) {
    throw new Error(
      `[B5b2-prep] loadRemote(${AUDIT_REMOTE_KEY}) returned null — ` +
        `remote may not be reachable. Check MFE_AUDIT_URL.`,
    );
  }
 ...
 * safety net.
 */
const AuditAppLazy = createLazyRemoteModule('Audit', loadAuditRemote);

/**
 * Public export used by `lazy-routes.ts` when the on-demand canary is
 * active.  Wraps `AuditAppLazy` in a Suspense fallback so the parent
 ...
 * tests).
 */
export const AuditAppOnDemand: FC = () => (
  <Suspense fallback={null}>
    <AuditAppLazy />
  </Suspense>
);
AuditAppOnDemand.displayName = 'AuditAppOnDemand';
 ...
 */
export const AUDIT_ON_DEMAND_BUILD_FLAG = __MFE_ADMIN_REMOTES_ON_DEMAND__;
