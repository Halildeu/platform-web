 ...
 *
 *   1. `apps/mfe-shell/vite.config.ts` omits the `mfe_access` entry from
 ...
 *      `import('mfe_access/AccessApp')` specifier for the
 *      `AccessAppOnDemand` component declared below.
 *   3. `shell-services-wiring.ts` swaps the static
 *      `import('mfe_access/shell-services')` line for an idle-deferred
 *      `ensureRemoteShellServicesConfigured('mfe_access', ...)` call so
 *      the shell-services contract (notify / SSE / impersonation / auth
 *      ready) is still wired post-auth without the eager fetch.
 *   4. On first navigation to `/access`, the lazy factory FIRST awaits
 ...
 *      `mfe_access/AccessApp` from the (now registered) remote.
 *
 ...
 * `import('mfe_access/AccessApp')` specifier no longer exists in the
...
const ACCESS_HOST_NAME = 'mfe_shell';
const ACCESS_REMOTE_NAME = 'mfe_access';
const ACCESS_REMOTE_KEY = `${ACCESS_REMOTE_NAME}/AccessApp`;
...
function getHostMfInstance(): MfHostInstance | null {
  ...
  return instances.find((i) => i.options.name === ACCESS_HOST_NAME) ?? null;
}

/**
 * Read the access remoteEntry URL from runtime env injected by the
 * index.html transform.  Lookup order matches the build-time
 * `MFE_ACCESS_URL` env that `vite.config.ts` reads for the eager mode
 * federation manifest (port 3005 default — see `remoteEntries.access`).
 */
function resolveAccessRemoteEntry(): string {
  if (typeof window !== 'undefined') {
    const w = window as Window & { __env__?: Record<string, string> };
    const url = w.__env__?.MFE_ACCESS_URL ?? w.__env__?.VITE_MFE_ACCESS_URL ?? null;
    if (url) return url;
  }
  if (typeof process !== 'undefined' && process.env) {
    const url = process.env.MFE_ACCESS_URL ?? process.env.VITE_MFE_ACCESS_URL ?? null;
    if (url) return url;
  }
  // Dev fallback — matches default in apps/mfe-shell/vite.config.ts.
  return 'http://localhost:3005/remoteEntry.js';
}
 ...
 *      — registers the remote + loads `mfe_access/shell-services` +
 ...
 *   3. `host.loadRemote('mfe_access/AccessApp')` — remote is now
 ...
 * pattern).
 */
async function loadAccessRemote(): Promise<{ default: FC<PropsWithChildren> }> {
  const sharedServices = getSharedShellServices();
  await ensureRemoteShellServicesConfigured(
    ACCESS_REMOTE_NAME,
    resolveAccessRemoteEntry(),
    sharedServices,
  );
  ...
  if (!mod) {
    throw new Error(
      `[B5b2-prep] loadRemote(${ACCESS_REMOTE_KEY}) returned null — ` +
        `remote may not be reachable. Check MFE_ACCESS_URL.`,
    );
  }
 ...
 * safety net.
 */
const AccessAppLazy = createLazyRemoteModule('Access', loadAccessRemote);

/**
 * Public export used by `lazy-routes.ts` when the on-demand canary is
 * active.  Wraps `AccessAppLazy` in a Suspense fallback so the parent
 ...
 * tests).
 */
export const AccessAppOnDemand: FC = () => (
  <Suspense fallback={null}>
    <AccessAppLazy />
  </Suspense>
);
AccessAppOnDemand.displayName = 'AccessAppOnDemand';
 ...
 */
export const ACCESS_ON_DEMAND_BUILD_FLAG = __MFE_ADMIN_REMOTES_ON_DEMAND__;
