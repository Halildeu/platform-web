...
/**
 * PERF-INIT-V2 PR-B5b2-prep-2 — `mfe_access` on-demand bootstrap canary
 ...
 *      `host.registerRemotes` + `host.loadRemote('mfe_access/shell-services')`
 *      + `mod.configureShellServices(sharedServices)`, THEN the wrapper
 *      calls `host.loadRemote('mfe_access/AccessApp')` to render the
 *      route component.
 ...
 *   6. Module-shape: `displayName`, `ACCESS_ON_DEMAND_BUILD_FLAG`.
 *   7. `resolveAccessRemoteEntry` env precedence.
 *
...
function installFakeHost(opts?: {
  ...
  const fake: FakeHostInstance = {
    ...
    loadRemote: vi.fn(async (key: string) => {
      ...
      // `${remoteName}/AccessApp` key
      if (opts?.appComponentOverride === null) return null;
      return {
        default:
          opts?.appComponentOverride ??
          (() => <div data-testid="access-remote-loaded">AccessApp loaded</div>),
      };
    }),
  };
...
describe('createAccessAppOnDemand (PR-B5b2-prep-2)', () => {
  beforeEach(async () => {
  ...
  it('first mount: ensure helper registers + loads shell-services + configures, then wrapper loads AccessApp', async () => {
    const host = installFakeHost();
    const mod = await import('./createAccessAppOnDemand');

    render(
      <Suspense fallback={<div>Loading</div>}>
        <mod.AccessAppOnDemand />
      </Suspense>,
    );

    expect(await screen.findByTestId('access-remote-loaded')).toBeInTheDocument();

    // Helper calls registerRemotes ONCE with the access entry.
    ...
    // loadRemote called twice — shell-services (via helper), then AccessApp (via wrapper).
    expect(host.loadRemote).toHaveBeenCalledWith('mfe_access/shell-services');
    expect(host.loadRemote).toHaveBeenCalledWith('mfe_access/AccessApp');
    expect(host.loadRemote).toHaveBeenCalledTimes(2);

    // Helper's configured-remotes Set marks mfe_access as configured.
    expect(await isConfigured('mfe_access')).toBe(true);
  });
  ...
  it('idempotent: re-mount does NOT re-trigger ensure helper register', async () => {
    ...
    await screen.findByTestId('access-remote-loaded');
    unmount();
  ...
  it('missing host instance surfaces classified remote-unavailable fallback', async () => {
    ...
    expect(await screen.findByTestId('remote-module-fallback-access')).toBeInTheDocument();
    expect(warnSpy).toHaveBeenCalled();
    // Failure does NOT mark the remote as configured — retry can succeed.
    expect(await isConfigured('mfe_access')).toBe(false);
  });
  ...
  it('AccessAppOnDemand has correct displayName', async () => {
    const mod = await import('./createAccessAppOnDemand');
    expect(mod.AccessAppOnDemand.displayName).toBe('AccessAppOnDemand');
  });

  it('exports ACCESS_ON_DEMAND_BUILD_FLAG', async () => {
    const mod = await import('./createAccessAppOnDemand');
    expect('ACCESS_ON_DEMAND_BUILD_FLAG' in mod).toBe(true);
  });

  it('uses VITE_MFE_ACCESS_URL when set', async () => {
    process.env.VITE_MFE_ACCESS_URL = 'http://example.test/custom-remote-entry.js';
    const host = installFakeHost();
    const mod = await import('./createAccessAppOnDemand');
  ...
  it('runtime MFE_ACCESS_URL takes precedence over VITE_MFE_ACCESS_URL', async () => {
    process.env.MFE_ACCESS_URL = 'http://runtime.example.test/remoteEntry.js';
    process.env.VITE_MFE_ACCESS_URL = 'http://build.example.test/remoteEntry.js';
    const host = installFakeHost();
    const mod = await import('./createAccessAppOnDemand');
  ...
  it('falls back to localhost:3005 when no env URL is set', async () => {
    delete process.env.MFE_ACCESS_URL;
    delete process.env.VITE_MFE_ACCESS_URL;
    const host = installFakeHost();
    const mod = await import('./createAccessAppOnDemand');
    ...
    const [remotes] = host.registerRemotes.mock.calls[0];
    expect(remotes[0].entry).toBe('http://localhost:3005/remoteEntry.js');
  });
});
