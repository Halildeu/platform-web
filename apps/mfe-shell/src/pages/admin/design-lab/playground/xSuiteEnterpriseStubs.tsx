/* eslint-disable @typescript-eslint/no-explicit-any -- dev-only Design Lab preview stubs; props shapes are scaffolded for visual mock-up, not type-strict. */
import React from 'react';
import { _xSuiteComponents } from './xSuiteStubs';

/* ---- Enterprise stubs ---- */
// AgingBuckets stub removed — component deleted in Phase 1b (PR #492)
...
_xSuiteComponents.ApprovalWorkflow = function ApprovalWorkflowStub(props: any) {
  ...
  return React.createElement(
    ...
    React.createElement(
      'div',
      { style: { fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 12 } },
      props.title || 'Onay Is Akisi',
    ),
    React.createElement(
      'div',
      ...
      ...steps.flatMap((s: any, i: number) => {
        ...
        const items = [
          React.createElement(
            'div',
            { key: s.id, style: { textAlign: 'center' as const, flex: 1 } },
            React.createElement(
              'div',
              {
                style: {
                  ...
                  background:
                    s.status === 'completed' ? statusColors.completed : 'var(--surface-muted)',
                  color:
                    s.status === 'completed' ? 'var(--surface-default)' : statusColors[s.status],
                  fontSize: 14,
                  fontWeight: 700,
          ...
          items.push(
            React.createElement('div', {
              ...
              style: {
                ...
                height: 2,
                background:
                  s.status === 'completed' ? statusColors.completed : 'var(--border-subtle)',
                maxWidth: 40,
              },
            }),
          );
...
_xSuiteComponents.BulletChart = function BulletChartStub(props: any) {
  ...
  const ranges = props.ranges || [
    ...
    { value: max, color: 'var(--state-success-bg))' },
  ];
  return React.createElement(
    'div',
    {
        ...
        React.createElement(
          ...
          React.createElement('span', {
            style: {
              ...
              height: 6,
              borderRadius: '50%',
              background: levelColors[e.level] || levelColors.info,
              flexShrink: 0,
            },
          }),
          React.createElement(
            'span',
            { style: { fontWeight: 500, color: 'var(--text-primary)', flex: 1 } },
            e.action,
          ),
          React.createElement(
            'span',
        ...
        React.createElement(
          'div',
          {
            ...
            style: {
              ...
              padding: '6px 10px',
              borderRadius: 8,
              background: t.active ? 'var(--surface-muted)' : 'transparent',
              cursor: 'pointer',
              border: t.active ? '1px solid var(--action-primary))' : '1px solid transparent',
            },
          },
          React.createElement(
            'div',
          ...
          t.active &&
            React.createElement(
              'span',
              { style: { marginLeft: 'auto', fontSize: 10, color: 'var(--action-primary))' } },
              '\u2713',
            ),
        ),
      ),
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        props.title || 'Baslangic Rehberi',
      ),
      React.createElement(
        'span',
        { style: { fontSize: 11, color: 'var(--text-secondary)' } },
        `${completed}/${items.length} tamamlandi`,
      ),
    ),
    React.createElement(
      'div',
      {
        style: { height: 4, borderRadius: 2, background: 'var(--surface-muted)', marginBottom: 12 },
      },
      React.createElement('div', {
        style: {
          height: '100%',
    ...
    React.createElement(
      ...
      ...items.map((item: any) =>
        React.createElement(
          'div',
          { key: item.id, style: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 } },
          React.createElement(
            'span',
            {
              style: {
                ...
                alignItems: 'center',
                justifyContent: 'center',
                background: item.completed ? 'var(--state-success-text))' : 'var(--surface-muted)',
                color: item.completed ? 'var(--surface-default)' : 'var(--text-secondary)',
                fontSize: 10,
                fontWeight: 600,
                border: item.completed ? 'none' : '1px solid var(--border-subtle)',
              },
            },
            item.completed ? '\u2713' : '',
          ),
...
_xSuiteComponents.PermissionMatrix = function PermissionMatrixStub(props: any) {
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      props.title || 'Yetki Matrisi',
    ),
    React.createElement(
      'table',
      { style: { width: '100%', borderCollapse: 'collapse' as const, fontSize: 12 } },
      React.createElement(
        'thead',
        null,
        React.createElement(
        ...
        ...permissions.map((p: any, i: number) =>
          React.createElement(
            'tr',
            {
              ...
              style: {
                borderBottom:
                  i < permissions.length - 1 ? '1px solid var(--border-subtle)' : 'none',
              },
            },
            React.createElement(
              'td',
              { style: { padding: '8px 16px', fontWeight: 500, color: 'var(--text-primary)' } },
              p.label,
            ),
            ...p.values.map((v: boolean, j: number) =>
              React.createElement(
                'td',
                { key: j, style: { padding: '8px 16px', textAlign: 'center' as const } },
                React.createElement(
                  'span',
                  {
                    style: {
                      color: v ? 'var(--state-success-text))' : 'var(--state-error-text))',
                      fontSize: 14,
                    },
                  },
                  v ? '\u2713' : '\u2717',
          ...
          React.createElement(
            'div',
            {
              ...
              style: {
                ...
                padding: '8px 12px',
                borderRadius: 8,
                border: `1px solid ${statusColors[s.status]}20`,
                background: `${statusColors[s.status]}08`,
                textAlign: 'center' as const,
              },
            },
            React.createElement(
              'div',
              { style: { fontSize: 11, fontWeight: 600, color: statusColors[s.status] } },
              s.label,
            ),
            s.duration &&
              React.createElement(
        ...
        if (i < stages.length - 1) {
          items.push(
            React.createElement(
              ...
              {
                key: `arrow-${i}`,
                style: { padding: '0 4px', color: 'var(--text-secondary)', fontSize: 14 },
              },
              '\u2192',
            ),
          );
...
_appSidebarStub.AppSidebar = function AppSidebarStub() {
  ...
  return React.createElement(
    ...
    React.createElement(
      'div',
      { style: { flex: 1, overflowY: 'auto' as const, padding: collapsed ? '8px 4px' : '8px' } },
      ...groups.map((group) =>
        React.createElement(
          'div',
          { key: group, style: { marginBottom: 12 } },
          !collapsed &&
            React.createElement(
              'div',
              {
                  ...
                  React.createElement(
                    'span',
                    {
                      style: {
                        ...
                        borderRadius: 10,
                        background:
                          item.badge === 'new' ? 'var(--action-primary)' : 'var(--surface-muted)',
                        color:
                          item.badge === 'new' ? 'var(--surface-default)' : 'var(--text-secondary)',
                        fontWeight: 600,
                      },
                    },
                    item.badge,
