...
function useVisualRegressionData() {
  ...
  return useMemo(() => {
    ...
    const evidenceStatus: EvidenceStatus = evidenceAvailable ? evidence.status : 'no_data';
    // K2-2: provenance registry'den gelir; loaded değilse 'no_data' (FAKE guarantee).
    const provenance = getEvidenceProvenance(
    ...
    const defaultVRStatus: VRStatus =
      evidenceStatus === 'passing' ? 'pass' : evidenceStatus === 'failing' ? 'fail' : 'skipped';
    ...
    const acceptanceRate = totalSnapshots > 0 ? Math.round((passCount / totalSnapshots) * 100) : 0;
...
> = {
  pass: {
    ...
    icon: <CheckCircle2 className="h-3.5 w-3.5 text-state-success-text" />,
  },
  fail: {
    label: 'Fail',
    color: 'text-state-danger-text',
    bg: 'bg-state-danger-bg',
    icon: <XCircle className="h-3.5 w-3.5 text-state-danger-text" />,
  },
  changed: {
    label: 'Changed',
    color: 'text-state-warning-text',
    bg: 'bg-state-warning-bg',
    icon: <AlertTriangle className="h-3.5 w-3.5 text-state-warning-text" />,
  },
  new: {
    label: 'New',
    color: 'text-state-info-text',
    bg: 'bg-state-info-bg',
    icon: <Eye className="h-3.5 w-3.5 text-action-primary" />,
  },
  skipped: {
    label: 'Skipped',
    color: 'text-[var(--text-secondary)]',
    bg: 'bg-[var(--surface-muted)]',
    icon: <Clock className="h-3.5 w-3.5 text-[var(--text-subtle)]" />,
  },
};
    ...
    <div className="flex flex-col mx-auto max-w-5xl gap-6 pb-16">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <DataProvenanceBadge level={data.provenance} />
            </div>
      ...
      {data.evidenceStatus === 'no_data' && (
        <div className="flex items-center gap-3 rounded-2xl border border-state-warning-text/20 bg-state-warning-bg px-5 py-3">
          ...
          <Text className="text-sm text-state-warning-text">
            ...
            <code className="rounded-xs bg-state-warning-bg px-1 text-xs">
      ...
      {data.evidenceStatus === 'configured' && (
        <div className="flex items-center gap-3 rounded-2xl border border-state-info-text/20 bg-state-info-bg px-5 py-3">
          ...
          <Text className="text-sm text-action-primary">
            Yapilandirilmis — henuz kosulmadi. CI pipeline ilk calistirildiginda sonuclar burada
            gorunecek.
          </Text>
        </div>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <StatCard
          ...
          color="text-state-warning-text"
          icon={<AlertTriangle className="h-4 w-4 text-state-warning-text" />}
        />
        <StatCard
          label="Failed"
        ...
        <StatCard
          ...
          icon={<Image className="h-4 w-4 text-text-secondary" />}
        />
      ...
      <div className="rounded-2xl border border-border-subtle bg-surface-default">
        ...
        <div className="border-t border-border-subtle px-4 py-2">
          <Text variant="secondary" className="text-[11px]">
            Showing {filteredResults.length} of {data.results.length} components ·{' '}
            {data.totalSnapshots} total snapshots
          </Text>
        </div>
      </div>
    </div>
