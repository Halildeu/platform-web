...
export interface ReportModule<TFilters extends Record<string, unknown>, TRow> {
  ...
  /**
   * PR-0.1+ capabilities envelope returned alongside metadata. ReportPage
   * reads {@code serverSideGrouping} to decide whether to expose the
   * row-group panel + drag-to-group + value-aggregation pickers. The
   * field lists tell the grid which columns participate so per-column
   * actions (column header menu Group action, QuickGroupMenu) can stay
   * gated. Modules whose data source predates the capability envelope
   * may return {@code undefined}; ReportPage treats that as all-false.
   */
