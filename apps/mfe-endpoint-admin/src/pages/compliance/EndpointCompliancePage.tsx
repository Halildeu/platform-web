 ...
 * cross-device list returns a deviceId we don't have full metadata for
 * yet. The drawer's DetayTab and similar components handle null /
 ...
 * once those load.
 */
function buildStubDevice(item: ComplianceDeviceListItem): EndpointDevice {
  return {
    id: item.deviceId,
    hostname: item.hostname ?? item.deviceId,
    displayName: null,
    osType: 'UNKNOWN',
    osVersion: null,
    agentVersion: null,
...
export const EndpointCompliancePage: React.FC = () => {
  ...
  const selectedDevice = React.useMemo<EndpointDevice | null>(() => {
    ...
    return fromCompliance ? buildStubDevice(fromCompliance) : null;
  }, [data, deviceList, selectedDeviceId]);
          ...
          <table className="compliance-list__table" data-testid="compliance-list-table">
            ...
            <tbody>
              {data.items.map((item) => (
                ...
                >
                  <td className="compliance-list__hostname">{item.hostname ?? item.deviceId}</td>
                  <td>
                  ...
                  <td>
                    <span className={stalenessChipClass(item.worstStaleness)}>
                      {stalenessLabel(item.worstStaleness, t)}
                    </span>
                  </td>
                  <td>
                    {item.policyDrift ? (
