...
export const KPICard = React.forwardRef<HTMLDivElement, KPICardProps>(function KPICard(
  ...
  return (
    <div
      ref={forwardedRef}
      className={cn(
        // Faz 21.10 wave 2: mobile-first padding so cards in 4-up
        // dashboards don't waste 32% of their width on chrome on phones.
    ...
    >
      ...
      <div className="mt-2 flex items-end gap-2">
        {/*
          Faz 21.10 wave 2: mobile-first value font. text-xl on mobile
          so 4 KPI cards fit on a 375px viewport without truncating;
          text-2xl on sm+ so the metric still feels heavy on tablet.
        */}
        <Text
          as="div"
      ...
      {/* ---- subtitle ---- */}
      {subtitle && (
        <Text className="mt-1 text-[11px] text-[var(--text-secondary)]">{subtitle}</Text>
      )}
