...
/* ------------------------------------------------------------------ */
/*  Mocks                                                              */
/*  - x-charts internal cn / Text are used unmocked.                   */
/*  - SparklineChart kept mocked so this remains a focused unit test   */
/*    of StatWidget composition (avoids ECharts setup overhead).       */
/* ------------------------------------------------------------------ */
...
describe('StatWidget', () => {
  ...
  it('formats number with precision', () => {
    render(<StatWidget label="Score" value={1234.567} precision={2} />);
  ...
  it('formats as currency with prefix', () => {
    render(<StatWidget label="Revenue" value={50000} format="currency" precision={2} prefix="$" />);
  ...
  it('formats as percent with suffix', () => {
    render(<StatWidget label="Conversion" value={0.1234} format="percent" precision={1} />);
  ...
  it('applies suffix', () => {
    render(<StatWidget label="Latency" value={42} suffix="ms" />);
  ...
  it('calculates change from previousValue', () => {
    ...
    render(<StatWidget label="Orders" value={150} previousValue={100} />);
  ...
  it('shows positive change with success color', () => {
    const { container } = render(<StatWidget label="Orders" value={150} previousValue={100} />);
  ...
  it('shows negative change with error color', () => {
    ...
    const { container } = render(<StatWidget label="Orders" value={50} previousValue={100} />);
  ...
  it('does not show change when previousValue is not provided', () => {
    const { container } = render(<StatWidget label="Orders" value={150} />);
  ...
  it('renders sparkline when sparkData provided', () => {
    render(<StatWidget label="Revenue" value={5000} sparkData={[10, 20, 15, 25, 30]} />);
