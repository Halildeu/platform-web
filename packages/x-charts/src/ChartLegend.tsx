...
export const ChartLegend = React.forwardRef<HTMLDivElement, ChartLegendProps>(function ChartLegend(
  ...
  const visibleItems =
    maxItems != null && maxItems < items.length ? items.slice(0, maxItems) : items;

  const overflowCount = maxItems != null && maxItems < items.length ? items.length - maxItems : 0;
  ...
  return (
    ...
    >
      {visibleItems.map((item, i) => (
        ...
        >
          ...
          <span
            ...
            aria-hidden="true"
          />
          <Text as="span" className="text-xs text-[var(--text-secondary)]">
            {item.label}
          </Text>
          {item.value != null && (
            <Text as="span" className="text-xs font-semibold text-[var(--text-primary)]">
              {item.value}
            </Text>
          )}
        </div>
      ...
      {overflowCount > 0 && (
        <Text as="span" className="text-[11px] text-[var(--text-tertiary)]" role="listitem">
          +{overflowCount} more
        </Text>
      )}
    </div>
