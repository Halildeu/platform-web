...
export type SharedReportFilterKey =
  | 'search'
  | 'status'
  | 'level'
  | 'department'
...
const AUDIT_FEED_CAPABILITIES: readonly AuditFeedCapability[] = [
  ...
  },
  {
    ...
    description:
      'Open successful notification-preference replay records in the central audit feed.',
...
const SHARED_REPORT_CATALOG: readonly SharedReportCatalogItem[] = [
  ...
  },
  {
    ...
    description:
      'Cinsiyet, yas, egitim, kidem ve cesitlilik metrikleriyle insan kaynaklari demografik raporu.',
  ...
  },
  {
    ...
    filterParity: [{ key: 'search', label: 'Arama', supportedChannels: ['web'] }],
  ...
  },
  {
    ...
    description:
      'Mercer/WTW standardında kapsamlı ücretlendirme analizi — iç denge, cinsiyet eşitliği, yüzdelik dağılımlar, trend.',
    permissionCode: 'hr-read',
...
const AUDIT_SUMMARY_GROUPS: readonly AuditSummaryGroup[] = [
  {
    ...
    description:
      'Track central audit visibility for auth session creation before drilling into the protected route.',
  ...
  },
  {
    ...
    description:
      'Surface replay success and conflict totals for queued notification preference mutations.',
...
const OFFLINE_MUTATION_POLICIES: readonly OfflineMutationPolicyDescriptor[] = [
  {
    ...
    description:
      'Replay queued profile session-timeout writes with optimistic locking and audit coverage.',
  ...
  },
  {
    ...
    description:
      'Replay queued notification-preference writes with optimistic locking and audit coverage.',
...
export function buildDefaultOfflineMutationPolicy(
  ...
  return {
    ...
    title: `${kind} mutation`,
    description:
      'Fallback policy for queued writes that have not been promoted into the shared capability catalog yet.',
...
export function listSharedReportFiltersForChannel(
  ...
  return listSharedReportFilters(reportId).filter((item) =>
    item.supportedChannels.includes(channel),
  );
}
...
export function saveSharedReportFilter(
  ...
  const scopedExisting = snapshot.savedFilters.filter(
    (item) =>
      item.reportId === filter.reportId && item.channel === filter.channel && item.id !== filter.id,
  );
