...
describe('Steps (Browser)', () => {
  ...
  it('calls onChange when step is clicked', async () => {
    ...
    const screen = render(<Steps items={stepItems} current={0} onChange={onChange} />);
  ...
  it('renders data-component attribute', async () => {
    const screen = render(<Steps items={stepItems} current={0} />);
    const el = screen.container.querySelector('[data-component="steps"]');
    expect(el).not.toBeNull();
  });

  it('renders in vertical direction', async () => {
    const screen = render(<Steps items={stepItems} current={0} direction="vertical" />);
    await expect.element(screen.getByText('Information')).toBeVisible();
  });
  ...
  it('renders dot style variant', async () => {
    const screen = render(<Steps items={stepItems} current={1} dot />);
    await expect.element(screen.getByText('Information')).toBeVisible();
    await expect.element(screen.getByText('Review')).toBeVisible();
  });

  it('renders error status on current step', async () => {
    const screen = render(<Steps items={stepItems} current={1} status="error" />);
    await expect.element(screen.getByText('Review')).toBeVisible();
  });
});
