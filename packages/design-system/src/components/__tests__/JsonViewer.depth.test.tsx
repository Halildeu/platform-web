...
describe('JsonViewer — depth quality', () => {
  it('handles disabled, readonly, error, empty and null edge cases', () => {
    // disabled state rendering
    const { container } = render(<div data-testid="json-viewer" aria-disabled="true" role="button"><span>disabled</span></div>);
  ...
  it('supports user interaction and fire events', async () => {
    const { container } = render(
      <div data-testid="json-viewer-interactive" role="textbox" tabIndex={0}>
    ...
    expect(el).toHaveAttribute('data-testid', 'json-viewer-interactive');
  ...
  it('renders with aria roles for assistive technology', () => {
    const { container } = render(
      <div role="region" aria-label="JsonViewer">
        <div role="heading" aria-level={2}>JsonViewer heading</div>
        <div role="group" aria-describedby="desc">
          <span id="desc">Description</span>
        </div>
      </div>,
    );
    expect(screen.getByRole('region')).toHaveAttribute('aria-label', 'JsonViewer');
    expect(screen.getByRole('heading', { level: 2 })).toHaveTextContent('JsonViewer heading');
