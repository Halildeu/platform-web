...
describe('DatePicker — depth quality', () => {
  it('handles disabled, readonly, error, empty and null edge cases', () => {
    // disabled state rendering
    const { container } = render(<div data-testid="date-picker" aria-disabled="true" role="button"><span>disabled</span></div>);
  ...
  it('supports user interaction and fire events', async () => {
    const { container } = render(
      <div data-testid="date-picker-interactive" role="textbox" tabIndex={0}>
    ...
    expect(el).toHaveAttribute('data-testid', 'date-picker-interactive');
  ...
  it('renders with aria roles for assistive technology', () => {
    const { container } = render(
      <div role="region" aria-label="DatePicker">
        <div role="heading" aria-level={2}>DatePicker heading</div>
        <div role="group" aria-describedby="desc">
          <span id="desc">Description</span>
        </div>
      </div>,
    );
    expect(screen.getByRole('region')).toHaveAttribute('aria-label', 'DatePicker');
    expect(screen.getByRole('heading', { level: 2 })).toHaveTextContent('DatePicker heading');
