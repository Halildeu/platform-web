// @vitest-environment jsdom
// Generated depth test — regenerate: node scripts/ci/generate-depth-tests.mjs --write
import React from 'react';
import { afterEach, describe, expect, it, vi } from 'vitest';
import '@testing-library/jest-dom/vitest';
import { cleanup, render, screen } from '@testing-library/react';
import { GroupedCardGallery } from '../grouped-card-gallery/GroupedCardGallery';

afterEach(cleanup);

describe('GroupedCardGallery — depth', () => {
  describe('GroupedCardGallery — depth: basic resilience', () => {
    it('renders without crash', () => {
      const { container } = render(<GroupedCardGallery />);
      expect(container.firstElementChild).toBeTruthy();
    });

    it('renders twice without side effects', () => {
      const { container: c1 } = render(<GroupedCardGallery />);
      cleanup();
      const { container: c2 } = render(<GroupedCardGallery />);
      expect(c2.firstElementChild).toBeTruthy();
    });
  });
});
