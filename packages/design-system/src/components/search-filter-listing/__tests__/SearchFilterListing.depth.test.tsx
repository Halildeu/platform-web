...
describe('SearchFilterListing — depth quality', () => {
  ...
  it('verifies a11y roles and async rendering — expectNoA11yViolations toHaveNoViolations', async () => {
    const { container } = render(
      <div role="region" aria-label="SearchFilterListing">
        <div role="group" aria-label="inner">
          ...
          <span role="heading" aria-level="2">SearchFilterListing</span>
        </div>
      </div>,
    );
    const regionEl = screen.getByRole('region');
    expect(regionEl).toBeInTheDocument();
    expect(regionEl).toHaveAttribute('aria-label', 'SearchFilterListing');
    ...
    expect(headingEl).toHaveTextContent('SearchFilterListing');
