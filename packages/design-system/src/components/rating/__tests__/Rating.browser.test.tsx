...
describe('Rating (Browser)', () => {
  ...
  it('fires onValueChange when clicking a star', async () => {
    ...
    const screen = render(<Rating onValueChange={onValueChange} />);
    const stars = screen.container.querySelectorAll('[role="radio"]');
  ...
  it('clears value when clicking same star (allowClear)', async () => {
    const onValueChange = vi.fn();
    const screen = render(<Rating defaultValue={3} onValueChange={onValueChange} />);
    const stars = screen.container.querySelectorAll('[role="radio"]');
    (stars[2] as HTMLElement).click();
  ...
  it('blocks interaction in readonly mode', async () => {
    const onValueChange = vi.fn();
    const screen = render(<Rating defaultValue={2} access="readonly" onValueChange={onValueChange} />);
    const stars = screen.container.querySelectorAll('[role="radio"]');
  ...
  it('navigates with ArrowRight keyboard', async () => {
    const onValueChange = vi.fn();
    const screen = render(<Rating defaultValue={2} onValueChange={onValueChange} />);
    const group = screen.getByRole('radiogroup');
    group.element().focus();
  ...
  it('renders description labels', async () => {
    ...
    const screen = render(<Rating defaultValue={3} labels={labels} />);
