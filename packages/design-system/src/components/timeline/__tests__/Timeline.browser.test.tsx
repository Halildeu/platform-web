...
describe('Timeline (Browser)', () => {
  ...
  it('renders data-component attribute', async () => {
    const screen = render(<Timeline items={timelineItems} />);
    const el = screen.container.querySelector('[data-component="timeline"]');
  ...
  it('renders in alternate mode', async () => {
    const screen = render(<Timeline items={timelineItems} mode="alternate" />);
    await expect.element(screen.getByText('Created account')).toBeVisible();
  });

  it('renders nothing when access is hidden', async () => {
    const screen = render(<Timeline items={timelineItems} access="hidden" />);
    expect(screen.container.querySelector('[data-component="timeline"]')).toBeNull();
  });
  ...
  it('renders reversed order', async () => {
    const screen = render(<Timeline items={timelineItems} reverse />);
    await expect.element(screen.getByText('Created account')).toBeVisible();
    await expect.element(screen.getByText('Completed profile')).toBeVisible();
  });
});
