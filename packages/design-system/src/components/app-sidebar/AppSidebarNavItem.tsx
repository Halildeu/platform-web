 ...
 * @see AppSidebar
 */
export const AppSidebarNavItem = React.forwardRef<
  HTMLDivElement,
  AppSidebarNavItemPropsBase & AccessControlledProps
>(
  (
    {
      icon,
    ...
    const depthPadding = isCollapsed
      ? 'px-2'
      : depth === 0
        ? 'px-3'
        : depth === 1
    ...
    const content = (
      <>
        {activeIndicator}
        {icon && (
          <span className="flex shrink-0 items-center justify-center [&>svg]:h-[18px] [&>svg]:w-[18px]">
            {icon}
          </span>
        )}
...
export type AppSidebarNavItemProps = AppSidebarNavItemPropsBase;
