...
describe('ThemePreviewCard (Browser)', () => {
  ...
  it('renders unselected state without checkmark', async () => {
    const screen = render(<ThemePreviewCard selected={false} />);
    expect(screen.container.querySelector('.sr-only')).toBeNull();
  });
  ...
  it('applies custom className', async () => {
    const screen = render(<ThemePreviewCard className="my-preview" />);
    const el = screen.container.querySelector('.my-preview');
