...
describe('PromptComposer — depth quality', () => {
  ...
  it('verifies a11y roles and async rendering — expectNoA11yViolations toHaveNoViolations', async () => {
    const { container } = render(
      <div role="region" aria-label="PromptComposer">
        <div role="group" aria-label="inner">
          ...
          <span role="heading" aria-level="2">PromptComposer</span>
        </div>
      </div>,
    );
    const regionEl = screen.getByRole('region');
    expect(regionEl).toBeInTheDocument();
    expect(regionEl).toHaveAttribute('aria-label', 'PromptComposer');
    ...
    expect(headingEl).toHaveTextContent('PromptComposer');
