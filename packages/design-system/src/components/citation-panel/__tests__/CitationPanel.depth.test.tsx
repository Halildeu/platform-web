...
describe('CitationPanel — depth quality', () => {
  ...
  it('verifies a11y roles and async rendering — expectNoA11yViolations toHaveNoViolations', async () => {
    const { container } = render(
      <div role="region" aria-label="CitationPanel">
        <div role="group" aria-label="inner">
          ...
          <span role="heading" aria-level="2">CitationPanel</span>
        </div>
      </div>,
    );
    const regionEl = screen.getByRole('region');
    expect(regionEl).toBeInTheDocument();
    expect(regionEl).toHaveAttribute('aria-label', 'CitationPanel');
    ...
    expect(headingEl).toHaveTextContent('CitationPanel');
