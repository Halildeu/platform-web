...
describe('FloatButton (Browser)', () => {
  ...
  it('has accessible aria-label', async () => {
    const screen = await render(<FloatButton aria-label="Create new item" />);
    const trigger = screen.getByTestId('float-button-trigger');
    await expect.element(trigger).toBeVisible();
  });

  it('renders tooltip on hover', async () => {
    const screen = await render(<FloatButton tooltip="Quick action" />);
    const trigger = screen.getByTestId('float-button-trigger');
    await expect.element(trigger).toBeVisible();
  });
});
