...
describe('TourCoachmarks — depth quality', () => {
  ...
  it('verifies a11y roles and async rendering — expectNoA11yViolations toHaveNoViolations', async () => {
    const { container } = render(
      <div role="region" aria-label="TourCoachmarks">
        <div role="group" aria-label="inner">
          ...
          <span role="heading" aria-level="2">TourCoachmarks</span>
        </div>
      </div>,
    );
    const regionEl = screen.getByRole('region');
    expect(regionEl).toBeInTheDocument();
    expect(regionEl).toHaveAttribute('aria-label', 'TourCoachmarks');
    ...
    expect(headingEl).toHaveTextContent('TourCoachmarks');
