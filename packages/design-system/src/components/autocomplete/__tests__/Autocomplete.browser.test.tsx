...
describe('Autocomplete (Browser)', () => {
  it('renders with placeholder and combobox role', async () => {
    const screen = await render(<Autocomplete options={options} placeholder="Search fruit" />);
  ...
  it('shows filtered suggestions when typing', async () => {
    const screen = await render(<Autocomplete options={options} />);
    const input = screen.getByRole('combobox');
    await input.click();
  ...
  it('selects option on click and fires onChange', async () => {
    ...
    const screen = await render(<Autocomplete options={options} onChange={onChange} />);
    const input = screen.getByRole('combobox');
    await input.click();
  ...
  it('blocks interaction when disabled', async () => {
    const onChange = vi.fn();
    const screen = await render(<Autocomplete options={options} disabled onChange={onChange} />);
    const input = screen.getByRole('combobox');
  ...
  it('shows loading state instead of options', async () => {
    const screen = await render(<Autocomplete options={[]} loading />);
    const input = screen.getByRole('combobox');
    await input.click();
  ...
  it('shows error message when error prop is set', async () => {
    const screen = await render(<Autocomplete options={options} error="Required field" />);
