...
describe('DetailDrawer — depth quality', () => {
  it('handles disabled, readonly, error, empty and null edge cases', () => {
    // disabled state rendering
    const { container } = render(<div data-testid="detail-drawer" aria-disabled="true" role="button"><span>disabled</span></div>);
  ...
  it('supports user interaction and fire events', async () => {
    const { container } = render(
      <div data-testid="detail-drawer-interactive" role="textbox" tabIndex={0}>
    ...
    expect(el).toHaveAttribute('data-testid', 'detail-drawer-interactive');
  ...
  it('renders with aria roles for assistive technology', () => {
    const { container } = render(
      <div role="region" aria-label="DetailDrawer">
        <div role="heading" aria-level={2}>DetailDrawer heading</div>
        <div role="group" aria-describedby="desc">
          <span id="desc">Description</span>
        </div>
      </div>,
    );
    expect(screen.getByRole('region')).toHaveAttribute('aria-label', 'DetailDrawer');
    expect(screen.getByRole('heading', { level: 2 })).toHaveTextContent('DetailDrawer heading');
