...
describe('Select (Browser)', () => {
  ...
  it('renders placeholder as first disabled option', async () => {
    const screen = await render(<Select options={options} placeholder="Choose one" />);
  ...
  it('is disabled when disabled prop is set', async () => {
    const screen = await render(<Select options={options} disabled />);
    await expect.element(screen.getByRole('combobox')).toBeDisabled();
  });

  it('does not fire onChange when disabled', async () => {
    const onChange = vi.fn();
    const screen = await render(<Select options={options} disabled onChange={onChange} />);
  ...
  it('uses value prop for controlled mode', async () => {
    const screen = await render(<Select options={options} value="c" onChange={() => {}} />);
  ...
  it('sets aria-invalid when error is truthy', async () => {
    const screen = await render(<Select options={options} error />);
  ...
  it('disables select and shows spinner when loading', async () => {
    const screen = await render(<Select options={options} loading />);
    await expect.element(screen.getByRole('combobox')).toBeDisabled();
  ...
  it('renders disabled options', async () => {
    ...
    const screen = await render(<Select options={opts} />);
    await expect.element(screen.getByRole('combobox')).toBeVisible();
  });
