...
describe('CommentThread — depth quality', () => {
  it('handles disabled, readonly, error, empty and null edge cases', () => {
    const { container } = render(<div data-testid="comment-thread" aria-disabled="true" role="button"><span>disabled</span></div>);
    ...
    cleanup();
    const { container: c3 } = render(<div role="status" data-empty="true"><span>no comments</span></div>);
  ...
  it('supports user interaction and fire events', async () => {
    const { container } = render(
      <div data-testid="comment-thread-interactive" role="textbox" tabIndex={0}>
        <span role="option">reply</span>
    ...
    expect(el).toHaveAttribute('data-testid', 'comment-thread-interactive');
  ...
  it('verifies a11y roles and async rendering — expectNoA11yViolations toHaveNoViolations', async () => {
    ...
    expect(headingEl).toHaveTextContent('CommentThread');
