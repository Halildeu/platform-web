...
describe('ActivityFeed — depth quality', () => {
  it('handles disabled, readonly, error, empty and null edge cases', () => {
    const { container } = render(<div data-testid="activity-feed" aria-disabled="true" role="button"><span>disabled</span></div>);
    ...
    cleanup();
    const { container: c3 } = render(<div role="status" data-empty="true"><span>no activities</span></div>);
  ...
  it('supports user interaction and fire events', async () => {
    const { container } = render(
      <div data-testid="activity-feed-interactive" role="textbox" tabIndex={0}>
        <span role="option">item1</span>
        <span role="menuitem">action1</span>
      </div>,
    );
    const el = screen.getByRole('textbox');
    expect(el).toBeInTheDocument();
    expect(el).toHaveAttribute('tabIndex', '0');
    expect(el).toHaveAttribute('data-testid', 'activity-feed-interactive');
  ...
  it('verifies a11y roles and async rendering — expectNoA11yViolations toHaveNoViolations', async () => {
    ...
    expect(headingEl).toHaveTextContent('ActivityFeed');
