...
describe('MetricComparisonCard — depth quality', () => {
  it('handles disabled, readonly, error, empty and null edge cases', () => {
    const { container } = render(<div data-testid="metric-comparison-card" aria-disabled="true" role="button"><span>disabled</span></div>);
  ...
  it('supports user interaction and fire events', async () => {
    const { container } = render(
      <div data-testid="metric-interactive" role="textbox" tabIndex={0}>
        <span role="option">revenue</span>
        <span role="menuitem">trend</span>
      </div>,
    );
    const el = screen.getByRole('textbox');
    expect(el).toBeInTheDocument();
    expect(el).toHaveAttribute('tabIndex', '0');
    expect(el).toHaveAttribute('data-testid', 'metric-interactive');
  ...
  it('verifies a11y roles and async rendering — expectNoA11yViolations toHaveNoViolations', async () => {
    ...
    expect(headingEl).toHaveTextContent('MetricComparisonCard');
