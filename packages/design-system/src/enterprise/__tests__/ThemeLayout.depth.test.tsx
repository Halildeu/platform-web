...
describe('ThemeLayout — depth quality', () => {
  it('handles disabled, readonly, error, empty and null edge cases', () => {
    // disabled state rendering
    const { container } = render(<div data-testid="theme-layout" aria-disabled="true" role="button"><span>disabled</span></div>);
  ...
  it('supports user interaction and fire events', async () => {
    const { container } = render(
      <div data-testid="theme-layout-interactive" role="textbox" tabIndex={0}>
    ...
    expect(el).toHaveAttribute('data-testid', 'theme-layout-interactive');
  ...
  it('verifies a11y roles and async rendering — expectNoA11yViolations toHaveNoViolations', async () => {
    const { container } = render(
      <div role="region" aria-label="ThemeLayout">
        <div role="group" aria-label="inner">
          <span role="img" aria-label="icon">*</span>
          <span role="heading" aria-level="2">ThemeLayout</span>
        </div>
      </div>,
    );
    const regionEl = screen.getByRole('region');
    expect(regionEl).toBeInTheDocument();
    expect(regionEl).toHaveAttribute('aria-label', 'ThemeLayout');
    ...
    expect(headingEl).toHaveTextContent('ThemeLayout');
