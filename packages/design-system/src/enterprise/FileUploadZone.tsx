...
function getStatusColor(status: UploadedFile['status']): string {
  switch (status) {
    case 'complete': return 'var(--state-success-text, #16a34a)';
    case 'error': return 'var(--state-error-text, #dc2626)';
    default: return 'var(--state-info-text, #2563eb)';
  }
}
  ...
  return (
    <div
      className={cn(
        ...
        file.status === 'error'
          ? 'border-[var(--state-error-border,var(--state-error-text))] bg-[var(--state-error-bg,#ef444408)]'
          : 'border-[var(--border-subtle,#e5e7eb)] bg-[var(--surface-default)]',
      )}
      role="listitem"
      ...
      <div className="flex-1 min-w-0">
        ...
        {file.status === 'uploading' && file.progress != null && (
          <div className="mt-1.5">
            <div className="h-1 rounded-full bg-[var(--surface-muted,#e5e7eb)] overflow-hidden">
              <div
      ...
      {canInteract && onRemove && (
        <button
          ...
          className={cn(
            ...
            'text-[var(--text-tertiary)] hover:text-[var(--state-error-text)] hover:bg-[var(--state-error-bg,#ef444410)]',
    ...
    >
      ...
      <div
        className={cn(
          ...
          isDragOver
            ? 'border-[var(--state-info-text,#2563eb)] bg-[var(--state-info-bg,#3b82f610)]'
            : 'border-[var(--border-default,#d1d5db)] bg-[var(--surface-default)]',
          canInteract && !isMaxReached
            ? 'cursor-pointer hover:border-[var(--state-info-text,#2563eb)] hover:bg-[var(--surface-muted)]'
