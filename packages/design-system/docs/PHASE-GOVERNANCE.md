...
> **Tarih:** 2026-03-24
> **Konsolide görünüm:** [docs/ROADMAP.md](../../../docs/ROADMAP.md) — 4+ yol haritası belgesinin tek-doğru özeti. Bu dosya hâlâ F-fazları için **authoritative**.
...
| F5 — AI-First Leapfrog             | 🟡 KISMEN  | MCP 21 tool (hedef 20+ aşıldı), AI testing kismen, intelligent runtime SEED'lendi (useAdaptiveLayout + AdaptiveForm), kalan: VS Code ext, privacy audit, fallback test             |
| F6 — DX & Ecosystem                | 🟡 KISMEN  | 9 \*Block.tsx + 3 page template + CLI calisiyor (orijinal "48 block" stale); Astro/Starlight portal scaffold mevcut (icerik planned); Figma sync scriptleri var (CI entegre eksik) |
| F7 — Commercial Hardening          | 🟡 KISMEN  | Semver check + migration tooling tamamlandi, LTS/RFC yapilmadi                                                                                                                     |
| F8 — AI Runtime Intelligence       | 🟡 BASLADI | `intelligence/predictive-engine.ts` + `a11y-runtime-guardian.ts` + 3 CI script (pr-design-review, component-predictor, a11y-guardian) seed; gate enforcement aktif değil           |
...
- 41 enterprise bilesen tamamlandi (Mart sonrası ek bileşenlerle güncel sayım; orijinal 38 hedef aşıldı)
...
| Alt-Faz                   | Durum | Detay                                                                                                  |
| ------------------------- | ----- | ------------------------------------------------------------------------------------------------------ |
| 5A — AI Developer Copilot | 🟡    | MCP 21 tool (hedef 20+ aşıldı). CLI mevcut. VS Code extension yapilmadi.                               |
| 5B — AI-Powered Testing   | 🟡    | Kismen tamamlandi. Test generation pipeline calisiyor.                                                 |
| 5C — Intelligent Runtime  | 🟡    | useAdaptiveLayout + AdaptiveForm SEED'lendi. SmartDashboard v2 + AdaptiveForm v2 progression bekliyor. |

### Kalan Isler

| #    | Is                                                 | Effort |
| ---- | -------------------------------------------------- | ------ |
| F5.1 | MCP 21 tool ✅ (hedef 20+ aşıldı; satır kapatıldı) | —      |
...
| Alt-Faz                 | Durum | Detay                                                                                                                                                                              |
| ----------------------- | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 6A — Blocks Marketplace | 🟡    | 9 `*Block.tsx` + 3 page template (`packages/blocks/src/blocks/{admin,analytics,crud,dashboard}` + `templates/`); CLI calisiyor; orijinal "48 block" iddiası kanıtsız (sayım stale) |
| 6B — Public Docs Portal | 🟡    | Astro/Starlight scaffold mevcut (`packages/design-system/docs-portal/`); icerik + CI entegrasyon eksik.                                                                            |
| 6C — Figma Round-Trip   | 🟡    | `scripts/figma-sync.mjs` + `figma-sync-reverse.mjs` mevcut; CI entegre + token diff eksik.                                                                                         |
...
| Kriter               | Olcum                                   | Hedef | Durum                                                                                    |
| -------------------- | --------------------------------------- | ----- | ---------------------------------------------------------------------------------------- |
| Block sayisi         | Unique block components                 | ≥ 30  | ⚠️ 9 \*Block.tsx + 3 template (orijinal 48 iddiası stale; gerçek count repo doğrulamalı) |
...
## F8 — AI Runtime Intelligence 🟡 BASLADI (Sprint B aktive et)
...
| F3  | ✅ DONE    | 5,321                  | 45/60                 | —                                                                                               |
| F4  | ✅ DONE    | 7,200+                 | ~54/60                | —                                                                                               |
| F5  | 🟡 KISMEN  | 7,200+                 | ~54/60                | ⚠️ VS Code ext + privacy audit + fallback test eksik (MCP 21/20 ✅, runtime SEED ✅)            |
| F6  | 🟡 KISMEN  | 7,200+                 | ~54/60                | ⚠️ Docs portal icerik + CI entegre, Figma round-trip CI integrate eksik (scaffold + scripts ✅) |
| F7  | 🟡 KISMEN  | 7,200+                 | ~54/60                | ⚠️ LTS/RFC yok                                                                                  |
| F8  | 🟡 BASLADI | 7,200+ → Hedef: 7,500+ | ~54/60 → Hedef: 55/60 | ⚠️ AI review noise, runtime perf, gate enforcement aktif değil                                  |
