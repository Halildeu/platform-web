...
export const ThemePreviewCard: React.FC<ThemePreviewCardProps> = ({ selected = false, className }) => {
  return (
    ...
    >
      {selected ? (
        <div className="absolute right-1 top-1 flex h-4 w-4 items-center justify-center rounded-full bg-[var(--accent-primary)] text-[9px] font-bold text-[var(--text-inverse,white)]">
          ✓
        </div>
      ) : null}
      <div className="flex flex-col gap-1 rounded-lg border border-border-subtle bg-surface-panel px-2 py-2">
        ...
        <div className="mt-2 flex items-center justify-end">
          <div className="inline-flex items-center rounded-full bg-[var(--action-primary-bg,var(--accent-primary))] px-2 py-[2px] text-[9px] font-semibold text-[var(--text-inverse,white)]">
            Kaydet
          </div>
        </div>
      </div>
