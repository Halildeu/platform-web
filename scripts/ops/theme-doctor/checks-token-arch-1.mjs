...
export function register(ctx) {
  const { check, readSafe, readThemeCss, readThemeInlineCss, srgbToHex, parseCssVarsFlat, walkDir,
    ...
    THEME_CSS, THEME_EXTENSION_CSS, TOKEN_BRIDGE_CSS, TOKENS_CSS, THEME_INLINE_CSS, FIX_HINT } = ctx;
...
check('hardcoded-fallbacks', 'var() with hardcoded hex fallbacks that should use token references', () => {
  const scanFiles = [THEME_CSS, THEME_EXTENSION_CSS, readSafe(TOKEN_BRIDGE_CSS) ? TOKEN_BRIDGE_CSS : null, SHELL_INDEX_CSS].filter(Boolean);
  const violations = [];
...
check('undefined-classes', 'CSS classes used in components but not generated by TW4 or custom CSS', () => {
  ...
  let tw4Classes;
  try {
    const tw4Css = execFileSync(
      process.platform === 'win32' ? 'npx.cmd' : 'npx',
      ['--yes', '@tailwindcss/cli', '--input', SHELL_INDEX_CSS],
      {
        cwd: ROOT,
        encoding: 'utf-8',
        timeout: 60000,
  ...
  const customCss = [readThemeCss(), readSafe(SHELL_INDEX_CSS)].join('\n');
