...
import { buildTestToken, applyAuthState } from './utils/auth';

const USERS_LINK_LABEL = /Users|Kullanıcılar/;

const prepareAuth = async (page, baseURL: string | undefined, permissions: string[]) => {
  const session = buildTestToken(permissions);
...
test.describe('Admin Users auth flow', () => {
  test('admin@example.com kullanıcıları menüsünden 200 yanıtlı grid görür', async ({ page, baseURL }) => {
    const root = await prepareAuth(page, baseURL, ['VIEW_USERS']);
  ...
  test('profil eksik kullanıcı login ekranına düşmeden uyarı görür', async ({ page, baseURL }) => {
    ...
    await expect(page).toHaveURL(/\/admin\/users/);
    await responsePromise;
    await expect(page.getByText('Profiliniz henüz oluşturulmamış', { exact: false })).toBeVisible();
