...
> ⚠️ Bu dosya **bağımsız Foundation Trust Track** dökümanıdır (eski F0-F6 taksonomisi: Package Trust → SSR → Behavior → Consumer → Docs Trust → Quality at Scale → Adoption). PHASE-GOVERNANCE.md F0-F8 (ürün vizyonu) ile **isim çakışmaktadır ama içerik farklıdır** — header'daki eski "mirror" iddiası yanıltıcı.
> **Konsolide görünüm:** [docs/ROADMAP.md](../../ROADMAP.md) — F-numbering çakışması §6.2'de açıklandı.
