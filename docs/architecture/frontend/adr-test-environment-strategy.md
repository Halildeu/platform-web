...
- `cssom-canary-required` — Vitest browser provider, **curated manifest** (Tailwind 4 sentinel + ThemeProvider + 6 primitives: Button, Input, Checkbox, Switch, Radio, Card). Manifest is an explicit list in `package.json` `test:cssom:canary` rather than a glob, so the gate stays fast (≈1m20s) and additions are deliberate. PR-9 promoted Input/Checkbox/Switch to this list after PR-8 brought the cssom test count past the 5+ threshold; PR-10 added Radio (a11y fix follow-up — same sr-only-input + visible-proxy pattern as Checkbox); PR-11 added Card (variant-token cascade lock). New primitive cssom tests should be added to the manifest as the foundation expands, balanced against the canary's runtime budget.
