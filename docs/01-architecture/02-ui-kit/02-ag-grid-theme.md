...
3. Tasarım token eşlemesi
   - Ant Design ve kurum renkleri ile AG Grid varlıklarını eşleyin:
     ...
     :root {
       ...
       --ag-header-foreground-color: rgba(0, 0, 0, 0.88);
...
## 3) Kod Örneği (UI Kit internals)

> **Not (PR-C grid-rendering contract, 2026-05-31):** Aşağıdaki kod
> kontratın **kendi iç bileşeni** (`packages/design-system/.../GridShell`
> ve `EntityGridTemplate`) içindeki `AgGridReact` mount örneğidir.
> `apps/` altında raw `AgGridReact` import **YASAK** (CI hard gate
> `pnpm --filter mfe-reporting run test:grid-contract`). App-seviye
> tüketici `GridShell` / `EntityGridTemplate` üzerinden geçer.
> Canonical sözleşme:
> [`docs/architecture/grid-rendering.md`](../../architecture/grid-rendering.md).

```tsx
// packages/design-system/src/advanced/data-grid/GridShell.tsx (internals)
<AgGridReact
