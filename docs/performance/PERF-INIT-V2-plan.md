...
> detail.
>
> **Last update (v7):** 2026-05-13 (cumulative 3-canary measured on
> testai `BUILD_SHA=c1a398f`). **CUMULATIVE Pre-V2 → Post-3-canary**:
> /login transfer **-80.5%** (49.1 → 9.6 MB), decoded **-26.3%**
> (49.05 → 36.1 MB), heap **-34.3%** (242 → 159 MB), resources -44%
> (171 → 139), LCP -484ms (3,208 → 2,724 ms; **224ms from sektör iyi
> threshold 2,500ms**), FCP -480ms (3,188 → 2,708 ms), TBT 76ms
> (sektör iyi içinde), CLS 0.002 (sektör mükemmel ✓). See §11
> revision history.
...
| Route                                  | Metric         | Pre-V2 (2026-05-11) | Pre-B3b0 (testai `BUILD_SHA=f0c8e09`, pre-edge patch) | Post-B3b0 (testai `BUILD_SHA=f0c8e09`, post H2+gzip) | Post-B5b1 canary (testai `BUILD_SHA=df640525`, B5b1 ONLY canary) | **Cumulative 3-canary** (testai `BUILD_SHA=c1a398f`, B5b1+B5b1.5+B5b2a) | **TOTAL Delta** (Pre-V2 → cumulative) | Gap to leader target              |
| -------------------------------------- | -------------- | ------------------- | ----------------------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------- | ------------------------------------- | --------------------------------- |
| `/login` (cold-anonymous)              | transfer       | est. 49 MB          | **49,100 KB** (median of 5 Playwright runs)           | **13,134 KB**                                        | **11,476 KB**                                                    | **9,589 KB**                                                            | **-80.5%** 🟢🟢🟢                     | 12× over leader (800 KB)          |
| `/login` (cold-anonymous)              | decoded JS     | est. 49 MB          | **49,050 KB**                                         | **49,050 KB**                                        | **42,960 KB**                                                    | **36,144 KB**                                                           | **-26.3%** 🟢🟢                       | 12× over leader (3,000 KB)        |
| `/login` (cold-anonymous)              | heap usage     | not measured        | n/a (proxy: heapUsedMB)                               | 242 MB                                               | **190 MB**                                                       | **159 MB**                                                              | **-34.3%** ✓ (-83 MB heap)            | leader n/a                        |
| `/login` (cold-anonymous)              | protocol       | not measured        | 171× HTTP/1.1                                         | 171× h2                                              | **161× h2**                                                      | **139× h2**                                                             | h2 multiplexed + 32 less requests     | n/a                               |
| `/login` (cold-anonymous)              | resource count | est. ~250           | 171                                                   | 171                                                  | **161**                                                          | **139**                                                                 | **-44%** (-32 resources)              | ≤ 80 leader (1.7× over)           |
| `/login` (cold-anonymous)              | LCP            | not measured        | 3,208 ms                                              | 3,416 ms                                             | **3,072 ms**                                                     | **2,724 ms**                                                            | **-484 ms** (from B3b0 baseline)      | CWV good 2,500ms — **224ms over** |
| `/login` (cold-anonymous)              | FCP            | not measured        | 3,188 ms                                              | 3,400 ms                                             | **3,052 ms**                                                     | **2,708 ms**                                                            | **-480 ms**                           | CWV good 1,800 ms (over)          |
| `/login` (cold-anonymous)              | TBT            | not measured        | 96 ms                                                 | 104 ms                                               | **79 ms**                                                        | **76 ms**                                                               | -20 ms ✓                              | ≤ 50 ms (close, "iyi" içinde)     |
| `/login` (cold-anonymous)              | long tasks     | not measured        | 2 count / 196 ms total                                | (varyans)                                            | **1 count / 130 ms total**                                       | **1 count / 126 ms total**                                              | -1 count, -70 ms                      | ≤ 2 count, ≤ 600 ms ✓             |
| `/login` (cold-anonymous)              | CLS            | not measured        | 0.002                                                 | 0.002                                                | 0.004                                                            | **0.002**                                                               | excellent ✓                           | ≤ 0.05 — **MÜKEMMEL** ✓           |
| `/login` (cold-anonymous)              | TTFB           | not measured        | 44 ms                                                 | 40 ms                                                | 41 ms                                                            | **40 ms**                                                               | -4 ms                                 | excellent ✓                       |
