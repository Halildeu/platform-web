...
| Route                                  | Metric         | Pre-V2 (2026-05-11) | Pre-B3b0 (testai `BUILD_SHA=f0c8e09`, pre-2026-05-13 22:55) | Post-B3b0 (testai `BUILD_SHA=f0c8e09`, post-2026-05-13 22:55) | Delta (pre-B3b0 → post-B3b0) | Gap to leader target       |
| -------------------------------------- | -------------- | ------------------- | ----------------------------------------------------------- | ------------------------------------------------------------- | ---------------------------- | -------------------------- |
| `/login` (cold-anonymous)              | transfer       | not measured        | **49,100 KB** (median of 5 Playwright runs)                 | **13,134 KB**                                                 | **-73.3%** ✓                 | 16× over leader (800 KB)   |
| `/login` (cold-anonymous)              | decoded JS     | not measured        | **49,050 KB**                                               | **49,050 KB**                                                 | unchanged (B5b2 scope)       | 16× over leader (3,000 KB) |
| `/login` (cold-anonymous)              | protocol       | not measured        | 171× HTTP/1.1                                               | 171× h2                                                       | multiplexed ✓                | n/a                        |
| `/login` (cold-anonymous)              | LCP            | not measured        | 3,208 ms                                                    | 3,416 ms                                                      | varyans within 5-run sample  | CWV good ≤ 2,500 ms (over) |
| `/login` (cold-anonymous)              | FCP            | not measured        | 3,188 ms                                                    | 3,400 ms                                                      | varyans within 5-run sample  | CWV good ≤ 1,800 ms (poor) |
| `/login` (cold-anonymous)              | TBT            | not measured        | 96 ms                                                       | 104 ms                                                        | varyans                      | ≤ 50 ms (over)             |
| `/login` (cold-anonymous)              | CLS            | not measured        | 0.002                                                       | 0.002                                                         | unchanged (excellent)        | ≤ 0.05 (already excellent) |
| `/login` (cold-anonymous)              | TTFB           | not measured        | 44 ms                                                       | 40 ms                                                         | -9% ✓                        | already excellent          |
...
| PR-B5b1      | SHIPPED (PR #450)                     | MFE on-demand bootstrap canary for **`mfe_suggestions`** (per B5b0 finding). Build-time omit + global host-instance `registerRemotes` + `loadRemote` pattern. Cross-AI Codex peer-review chain on threads `019e228d` → `019e22a1` → `019e22a7` → `019e22ac` → `019e22b3` → `019e22bd` (6-iter AGREE; iter-1 RED P0-1 runtime instance scoping + P0-2 rollback semantic + P1 error fallback + P3 lockfile drift all absorbed). **MERGED 2026-05-13 19:20Z**. **Pending**: operator deploys testai with `VITE_MFE_ON_DEMAND_BOOTSTRAP=1` + Playwright measure `/home` decoded delta (depends on M2a auth-storage). **NOT credited with**: DS root wrapper size reduction (Codex iter-7 expectation reset). | Canary decoded -2 to -4 MB on remote bootstrap; informs B5b2 ramp                                             |
| PR-B5b2      | PLANNED                               | MFE on-demand rollout to admin remotes (`access`, `audit`, `users`, `reporting`, `schema-explorer`). Route-scoped bootstrap; remote loads only when its route is reachable. **Precondition**: BUILD-TIME flag still LIVE; B5b1 canary measurement closure landed. **Acceptance**: admin smoke matrix (`/admin/users`, `/admin/access/roles`, `/admin/audit/events`, `/admin/reports/*`) all render correctly + no shell-services contract regression (notifications, audit SSE, impersonation, auth-ready Promise).                                                                                                                                                                                      | `/home` decoded **-8 to -18 MB** (post-PR target: ~25-32 MB)                                                  |
...
| PR      | Status                                                       | Deliverable                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Expected ROI                                                                           |
| ------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------- |
| PR-B3b0 | SHIPPED (`platform-k8s-gitops` PR #550 in-flight 2026-05-13) | **Host edge HTTP/2 + gzip live patch + Codex 019ded8d drift reconcile**. Live patch applied + verified on staging-sw (curl ALPN h2 accept + Content-Encoding gzip + Playwright median-of-5 transfer 49.1 MB → 13.1 MB = -73.3%). Container restart required (bind-mount stale-inode bug). Repo canonical reconciles 2026-05-03 Codex `019ded8d` audit drift (ai.acik.com cluster-authoritative switch) + today's H2/gzip transport patch in a single commit. | **Transfer -73% on /login**; multiplexed h2; foundational for B3b1 Brotli              |
...
| PR                                                                    | Codex thread                                                               | Verdict                                                                                                                                                                                                      | Merged at (UTC)                                    |
| --------------------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------- |
| #436 PMD v2                                                           | `019e20fa` iter-2                                                          | REVISE-actionable absorbed                                                                                                                                                                                   | 2026-05-13T11:30Z                                  |
| #437 PR-B5c-lite                                                      | `019e20fa` iter-2 + `019e2112` iter-3                                      | AGREE (route-rendered sentinel + redirect guard + observer expose contract)                                                                                                                                  | 2026-05-13T12:50Z                                  |
| #438 PR-B5a                                                           | `019e20fa` iter-3                                                          | PARTIAL absorbed (consumer hygiene, ROI=0 — informed PMD v3)                                                                                                                                                 | 2026-05-13T13:55Z                                  |
| #443 PR-B5d0 + PMD v3 + PMD v4                                        | `019e20fa` iter-5 + iter-7                                                 | AGREE (B5d truncated; B5d-arch backlog added; B5b primary decoded path)                                                                                                                                      | 2026-05-13T14:34Z                                  |
| #444 PR-B5b3-prep                                                     | `019e2266` iter-1 (first-defined-wins precedence)                          | AGREE                                                                                                                                                                                                        | 2026-05-13T15:50Z                                  |
| #446 PR-B5b0 (re-open after #445)                                     | `019e20fa` iter-5 + `019e2272` iter-1                                      | AGREE (B5b0 initiator-trace + B5b1 strategy)                                                                                                                                                                 | 2026-05-13T18:33Z                                  |
| **#450 PR-B5b1**                                                      | `019e228d`→`019e22a1`→`019e22a7`→`019e22ac`→`019e22b3`→`019e22bd` (6-iter) | AGREE final (P0-1 runtime instance scoping via globalThis.**FEDERATION**.**INSTANCES** + P0-2 build-time-only rollback + P1 createLazyRemoteModule wrap + P3 lockfile clean + iter-5/6 repo-wide doc parity) | **2026-05-13T19:20Z**                              |
| **B3b0 host edge H2+gzip** (cross-repo `platform-k8s-gitops` PR #550) | `019e22df`                                                                 | AGREE-with-revisions (A3 sequencing); live patch verified + repo reconcile pending iter-2 review                                                                                                             | 2026-05-13T19:50Z (live patch) / PR #550 in-flight |
| PR-B5b1.5 (mfe_ethic — implicit)                                      | —                                                                          | —                                                                                                                                                                                                            | —                                                  |
| PR-B5b2                                                               | —                                                                          | —                                                                                                                                                                                                            | —                                                  |
| PR-B5b3                                                               | —                                                                          | —                                                                                                                                                                                                            | —                                                  |
...
**Running total: 19 platform-web PRs MERGED + 1 cross-repo (`platform-k8s-gitops` PR #550 in-flight)**. PR-S1 cross-repo tracked separately.
