...
>
> **Last update:** 2026-05-13 (autonomous run merged 12 PRs).
...
| PR-B3b | PLANNED (cross-repo) | HTTP/2 + Brotli at the ingress (platform-k8s-gitops nginx ingress overlay) |
| PR-B3c | PLANNED (cross-repo) | Long-cache headers for hashed assets, short-cache for HTML (platform-k8s-gitops) |
| PR-B3d | PLANNED (complex) | CSS critical extract + non-critical defer (consider `critters` or `vite-plugin-critical`) |
...
CI gates: `node scripts/ops/federation-doctor.mjs --json`
(`shared-deps-consistency`) and on-demand
`node scripts/diagnostics/mf-shared-keys.mjs [--json|--strict]`.
...
|---|---|---|
| HostOnly + import:false breaks if shell ships a different react-query version | high | pnpm-lock unification + same-deploy-set policy + mf-shared-keys --strict in CI |
...
| Lazy-loaded admin pages add chunk-fetch latency on admin first-visit | low | Tiny chunks (4–20 kB); behind admin click intent, not first paint |
...
| #434 PR-B4b | 019e20a0 | AGREE | (in-flight) |
