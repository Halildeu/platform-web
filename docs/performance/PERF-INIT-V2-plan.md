...
> detail.
>
> **Last update (v8):** 2026-05-14 (cumulative 4-canary measured on
> testai `BUILD_SHA=2a59704`). **CUMULATIVE Pre-V2 → Post-B5b2**:
> /login transfer **-95.2%** (49.1 → 2.34 MB), decoded **-81.5%**
> (49.05 → 9.09 MB), heap **-83.5%** (242 → 40 MB), resources **-62.6%**
> (171 → 64), **LCP -68.3% (3,208 → 1,016 ms — sektör MÜKEMMEL ✓✓✓
> <1,500ms threshold crossed)**, FCP -68.6% (3,188 → 1,000 ms),
> TBT 71ms (sektör iyi içinde), CLS 0.004 (sektör mükemmel ✓).
> /login transfer is **3× over leader 800 KB** (was 12× pre-B5b2);
> decoded is **3× over leader 3,000 KB** (was 12× pre-B5b2). See §11
> revision history.
...
| PR-B5b3 (federation-doctor CI gates)                                  | —                                                                          | —                                                                                                                                                                                                                                                                                     | — (queued)                                                     |
...
**Running total PERF-INIT-V2 platform-web PRs MERGED: 24** (Wave A+B closed = 12 from §9 above; Wave B5/B6 incremental = 12: #436, #437, #438, #443, #444, #446, #450, #453, #454, #455, #456, #458, #459, #460 — including PMD v5/v6/v7 doc PRs and B5b1 CI build infra). Plus 1 cross-repo MERGED: `platform-k8s-gitops` PR #550 (B3b0 edge canonical reconcile, 2026-05-13T20:17:08Z). PR-S1 cross-repo tracked separately in `platform-k8s-gitops` own audit trail. Non-PERF-INIT-V2 platform-web merges (e.g. #449 design-lab chart catalog) excluded from this count.
