...
> detail.
>
> **Last update (v6):** 2026-05-13 (post B5b1 CI canary deployed
>
> - MEASURED on testai + B5b1.5 ethic SHIPPED + B5b2a schema_explorer
>   SHIPPED + B5b2b-e architectural blocker documented). B5b1 canary
>   beat ROI expectation: -12.4% decoded, -12.6% transfer, -21.5% heap,
>   -136ms LCP/FCP measured. See §11 revision history.
...
| Route                                  | Metric         | Pre-V2 (2026-05-11) | Pre-B3b0 (testai `BUILD_SHA=f0c8e09`, pre-edge patch) | Post-B3b0 (testai `BUILD_SHA=f0c8e09`, post H2+gzip) | Post-B5b1 canary (testai `BUILD_SHA=df640525`, B5b1 ONLY canary) | Delta (pre-B3b0 → B5b1 canary) | Gap to leader target       |
| -------------------------------------- | -------------- | ------------------- | ----------------------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------- | ------------------------------ | -------------------------- |
| `/login` (cold-anonymous)              | transfer       | not measured        | **49,100 KB** (median of 5 Playwright runs)           | **13,134 KB**                                        | **11,476 KB**                                                    | **-76.6%** ✓                   | 14× over leader (800 KB)   |
| `/login` (cold-anonymous)              | decoded JS     | not measured        | **49,050 KB**                                         | **49,050 KB**                                        | **42,960 KB**                                                    | **-12.4%** ✓ (-6 MB)           | 14× over leader (3,000 KB) |
| `/login` (cold-anonymous)              | heap usage     | not measured        | n/a (proxy: heapUsedMB)                               | 242 MB                                               | **190 MB**                                                       | **-21.5%** ✓ (-52 MB)          | leader target n/a          |
| `/login` (cold-anonymous)              | protocol       | not measured        | 171× HTTP/1.1                                         | 171× h2                                              | **161× h2**                                                      | multiplexed + 10 less requests | n/a                        |
| `/login` (cold-anonymous)              | resource count | not measured        | 171                                                   | 171                                                  | **161**                                                          | -10 (suggestions chunks)       | ≤ 80 leader                |
| `/login` (cold-anonymous)              | LCP            | not measured        | 3,208 ms                                              | 3,416 ms                                             | **3,072 ms**                                                     | **-136 ms** ✓                  | CWV good ≤ 2,500 ms (over) |
| `/login` (cold-anonymous)              | FCP            | not measured        | 3,188 ms                                              | 3,400 ms                                             | **3,052 ms**                                                     | **-136 ms** ✓                  | CWV good ≤ 1,800 ms (poor) |
| `/login` (cold-anonymous)              | TBT            | not measured        | 96 ms                                                 | 104 ms                                               | **79 ms**                                                        | **-17 ms** ✓                   | ≤ 50 ms (close)            |
| `/login` (cold-anonymous)              | long tasks     | not measured        | 2 count / 196 ms total                                | (varyans)                                            | **1 count / 130 ms total**                                       | -1 count, -66 ms               | ≤ 2 count, ≤ 600 ms        |
| `/login` (cold-anonymous)              | CLS            | not measured        | 0.002                                                 | 0.002                                                | 0.004                                                            | +0.002 (still excellent)       | ≤ 0.05                     |
| `/login` (cold-anonymous)              | TTFB           | not measured        | 44 ms                                                 | 40 ms                                                | 41 ms                                                            | -3 ms                          | already excellent          |
