...
| PR-B5b0      | SHIPPED (PR #445)                               | RemoteEntry initiator-attribution diagnostic (CDP `Network.requestWillBeSent`, captured on testai `/login` BUILD_SHA `08fb4b47` + `8885120f`). **Finding**: ALL 7 `/remotes/*/remoteEntry.js` fetches initiated from the shell's main app bundle (`dist-*.js` line 3 col 13645) — the `@module-federation/vite` plugin's host-side registration loop. NOT from shell-services-wiring (PR-B3a idle-deferred), preload helper, or lazy route loader. **B5b1 strategy update**: deferring shell-services-wiring alone cannot reduce remote bootstrap fetches; B5b1 must conditionally exclude opt-in remotes from `federation({remotes})` at build-time (via PR-B5b3-prep #444 flag) and use `@module-federation/runtime` `registerRemotes` API at route-loader level. **B5b1 canary recommendation**: `mfe_suggestions` (lowest blast — 0 direct react-query imports, single exposed module, NOT in shell-services-wiring's 4-remote list).                                                                                                                                                                                                                                                                                                              | Diagnostic data — informs B5b1; not user-facing perf delta itself                                             |
| PR-B5b1      | UNBLOCKED (B5d0 + B5b0 closed) — strategy known | MFE on-demand bootstrap canary for **`mfe_suggestions`** (per B5b0 recommendation). **Strategy** (Codex iter-1 thread `019e2272` per-remote conditional): when `MFE_ON_DEMAND_BOOTSTRAP=1`, omit `mfe_suggestions` from `federation({remotes})` in shell `vite.config.ts` AND remove the static `import('mfe_suggestions/SuggestionsApp')` specifier from `apps/mfe-shell/src/app/router/lazy-routes.ts`; replace with `@module-federation/runtime` `registerRemotes` + `loadRemote` call at the route-loader level. **Preconditions**: (1) `MFE_ON_DEMAND_BOOTSTRAP` rollback flag LIVE (B5b3-prep #444 in flight). (2) `@module-federation/runtime` added as explicit shell dependency (currently transitive only). (3) Use the same build-time omit pattern as `mfe_endpoint_admin` (`__SHELL_ENDPOINT_ADMIN_REMOTE_ENABLED__` precedent in vite.config.ts). **Acceptance**: flag off = remoteEntry count identical to baseline; flag on canary = `/home` remoteEntry count drops by EXACTLY 1 (suggestions); `/suggestions` route renders after runtime register; rollback flag toggle restores eager behaviour; browser smoke confirms no MF cycle / share-scope race regressions. **NOT credited with**: DS root wrapper size reduction. | Canary decoded -2 to -4 MB on remote bootstrap; informs B5b2 ramp                                             |
