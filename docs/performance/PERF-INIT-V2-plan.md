...
> detail.
>
> **Last update (v5):** 2026-05-13 (post B5b1 #450 SHIPPED + B3b0
> host edge H2+gzip live patch via cross-repo `platform-k8s-gitops`
> PR #550). Codex thread `019e22df` for B3b0; thread `019e22bd`
> final AGREE for B5b1 6-iter chain. See §11 revision history.
...
| PR      | Status                                                 | Deliverable                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Expected ROI                                                                           |
| ------- | ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| PR-B3b0 | LIVE-PATCH-VERIFIED + repo reconcile PR #550 in-flight | **Host edge HTTP/2 + gzip live patch + Codex 019ded8d drift reconcile**. Live patch applied + verified on staging-sw (curl ALPN h2 accept + Content-Encoding gzip + Playwright median-of-5 transfer 49.1 MB → 13.1 MB = -73.3%). Container restart required (bind-mount stale-inode bug — runbook learning: `sed -i` creates new inode, container retains old until restart). Repo canonical (`platform-k8s-gitops` PR #550) reconciles 2026-05-03 Codex `019ded8d` audit drift (ai.acik.com cluster-authoritative switch) + today's H2/gzip transport patch in a single commit. | **Transfer -73% on /login**; multiplexed h2; foundational for B3b1 Brotli              |
...
| **B3b0 host edge H2+gzip** (cross-repo `platform-k8s-gitops` PR #550) | `019e22df` (iter-1 AGREE-with-revisions, iter-2 AGREE)                     | A3 sequencing (parallel, edge first → H2+gzip live → B5b2 separate). Live patch verified on staging-sw; repo reconcile PR #550 CI 6/7 SUCCESS, awaiting boundary-declaration advisory fix.                                                                                            | 2026-05-13T19:50Z (live patch); PR #550 cross-repo in-flight |
...
**Running total PERF-INIT-V2 platform-web PRs MERGED: 19** (Wave A+B closed = 12 from §9 above; Wave B5/B6 incremental = 7: #436, #437, #438, #443, #444, #446, #450). Plus 1 cross-repo (`platform-k8s-gitops` PR #550 in-flight, B3b0 edge canonical reconcile). PR-S1 cross-repo tracked separately in `platform-k8s-gitops` own audit trail. Non-PERF-INIT-V2 platform-web merges (e.g. #449 design-lab chart catalog) excluded from this count.
