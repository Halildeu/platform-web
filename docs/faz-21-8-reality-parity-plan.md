...
> **Bu plan v3**: tüm bu drift'leri kapatır + "kanıt doğru şeyi ölçüyor mu?" T4 katmanını ekler. **7 PR (X0=plan + X1..X5 with X3+X4 split)**, ~27-43h tahmini. Per-PR doc inline güncellenir; X5 sadece final sweep + AST adoption scan.
...
| 3   | T4 perf hard absolute gate **agresif** (CI GPU'suz, font/layout flaky)                                                          | PR-X3b T4 perf revize: hard absolute gate yerine **baseline + regression**: PR gate baseline'a göre +20-30% regression fail; absolute soft cap 800ms p95 (alarm); ilk 1-2 PR report-only veri toplama; baseline stabilize olunca hard gate. Ölçüm noktası: ECharts `rendered` event veya custom `chart-ready` callback (Storybook hydration overhead'i hariç). Canvas vs SVG perf ayrı bütçe.                 |
...
| **Perf regression smoke**     | yok                             | Headless Chromium'da 13 chart × 5 theme initial render. Gate: baseline p95 + 25% regression fail; absolute soft cap 800ms p95 (alarm); ilk 1-2 PR report-only. Canvas vs SVG ayrı bütçe. Ölçüm noktası ECharts `rendered` event veya `chart-ready` callback. | X3b (next to runtime contrast) |
...
| ---------------- | ------------------------------------------------------------------------------------ | ---------- | ------------------------------------------------------- |
| **X0 (this PR)** | Plan v3 dosyası + Codex iter-3 final review (AGREE)                                  | 0.5h       | yok                                                     |
...
- `packages/x-charts/src/useRealTimeData.ts`:
  ...
  // Type guard (Codex iter-2/iter-3: user-defined type guard, cast yok):
  // Base tipte `tickIntervalMs?: undefined` declared olduğu için doğrudan
  // property okuma TS'de güvenli — cast pattern bırakıldı.
  function hasAutoTick<T>(o: RealTimeDataOptions<T>): o is RealTimeDataOptionsAutoTick<T> {
    return typeof o.tickIntervalMs === 'number';
  }
  ...
  // Runtime dev assertions (Codex iter-3: pozitif sonlu sayı + onTick zorunluluğu):
  useEffect(() => {
...
- (T4 perf smoke — **Codex iter-2 düzeltme**) `packages/x-charts/src/__tests__/render-perf.test.ts` (NEW) — Playwright browser test:
  ...
  - **Gate stratejisi**: hard absolute number değil, **baseline + regression** (Codex iter-2/iter-3 düzeltme):
...
## Sequencing & Dependencies (v3 — Codex iter-2 X4 split absorbed)

```
PR-X0 (plan v3)
  ↓
Codex iter-3 review (final)
  ↓
  ├── PR-X1 (T1 fakes: XLSX type-removal + realtime discriminated union) — independent, fast
  ├── PR-X2 (T2 drilldown redo expose + SSR subpath + 'use client' + T4 boundary) — independent
  ├── PR-X3a (T2 HC palette/token redesign) — independent, must merge before X3b
  │     ↓
  │   PR-X3b (T2 runtime contrast gate + T4 perf baseline+regression) — depends X3a
  ├── PR-X4a (T3 hr-compensation CrossFilterProvider migration + mfe-users AG Grid bridge) — independent, **paralel with X1/X2/X3a**
  └── PR-X4b (T3 context-health drill) — **conditional** (only if backend project→component→file data ready); depends X2 DrillDown surface
  ↓
PR-X5 (T3 final doc audit + T4 AST adoption scan) — last, depends X1..X4 merged
```

**Parallel batch** (after X0 AGREE): X1 + X2 + X3a + X4a — bağımsız scope, paralel implement edilir.
**X3b** X3a merged sonrası. **X4b** conditional: backend data hazır + X2 DrillDown surface merged sonrası. **X5** son.
...
| T4 perf                             | 13 chart × 5 theme baseline + regression gate (PR p95 + 25% fail), 800ms absolute soft cap, ilk 1-2 PR report-only, canvas vs SVG ayrı bütçe |
...
| ---------------- | ---------------- | -------------------------------------- | ----------------------------------- | ------ |
| **X0** (this PR) | in progress (v3) | iter-1+2 absorbed, iter-3 final review | `docs/faz-21-8-reality-parity-plan` | —      |
| X1               | pending          | —                                      | —                                   | —      |
| X2               | pending          | —                                      | —                                   | —      |
| X3a              | pending          | —                                      | —                                   | —      |
| X3b              | pending          | —                                      | —                                   | —      |
...
1. PR-X0 v3 commit + push (PR #174 update) — **DONE**
2. Codex iter-3 final review — bekliyoruz, AGREE bekleniyor
3. AGREE alınca **paralel batch** başla: PR-X1 + PR-X2 + PR-X3a + PR-X4a aynı anda
4. X3b X3a merged sonrası; X4b conditional (backend ready); X5 son

---

_Bu plan v3 2026-05-03'te oluşturuldu. v1 Codex iter-1 PARTIAL/REVISE/RED + v2 Codex iter-2 REVISE feedback'leri absorb edildi. FAZ 21.4 + Quality-Sprint M1-M4 kapanışı sonrasında x-charts feature parity tahkimatı için yazıldı. "Hiç fake yok" kullanıcı kuralı (No Fake Work HARD RULE) bu planın temel çıkış noktasıdır._
